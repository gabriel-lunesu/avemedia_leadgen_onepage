...
function Services({ lightMode }) {
  ...
  return (
    <section className="services section-padding pb-0 block-pattern">
          ...
          <div className="serv-swiper">
            {
              ...
              <Swiper {...swiperOptions} id="content-carousel-container-unq-serv">
                {
                  data.map((item, index) => (
                    <SwiperSlide key={item.id} className="wow fadeIn" data-wow-delay={`${(data.length - index + 1) * 0.2}s`}>
                      <div className="item">
      ...
      <div className="bg-pattern bg-img" data-background={`${lightMode ? '/light/' : '/dark/'}assets/imgs/patterns/bg-line-1.svg`}></div>
    </section>
  )
}
